    ...
    app.service('dialogService', ['$modal','$templateCache', function ($modal, $templateCache) {
        ...
        var defaultDialogProperties = {
            ...
            headerTemplate: function () {
                var type = this.context.myType && fo.utils.getType(this.context) + 'Header' + this.role;
                var templateUrl = this.context.headerUrl ? this.context.headerUrl : type;
                return templateUrl;
            },
            bodyTemplate: function () {
                var type = this.context.myType && fo.utils.getType(this.context) + 'Body' + this.role;
                var templateUrl = this.context.templateUrl ? this.context.templateUrl : type;
                return templateUrl;
            },
            footerTemplate: function () {
                var type = this.context.myType && fo.utils.getType(this.context) + 'Footer' + this.role;
        ...
        this.doPopupDialog = function (spec, actions, args) {
            ...
            var result = currentDialog = dialog.display(args);
            return result;
        }

        this.doCloseDialog = function (result) {
            if (currentDialog) currentDialog.close(result);
            currentDialog = undefined;
        }

        this.doDismissDialog = function (result) {
            if (currentDialog) currentDialog.dismiss(result);
            currentDialog = undefined;
        }

        var dialogService = this;
        //override a version that has no dialog
        ws.Workspace.prototype.userSaveFileDialog = function (onComplete, defaultExt, defaultValue) {
            var self = this;
